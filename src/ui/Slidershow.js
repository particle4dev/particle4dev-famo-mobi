...
define('famodev/ui/Slidershow', [
        ...
        for (i = (this.total - 1); i >= 0; i--) {
            ...
            if (typeof this.options.sliders[i].content === 'string') {
                ...
                if (typeof this.options.sliders[i].contentModifier === 'object') {
                    container.add(this.options.sliders[i].contentModifier)
                        .add(surf);
                }
                else {
                    mod = new Modifier({
    ...
    function _resetNavigation(slide) {
        ...
        nav.view._node._child[this._navigationDot].get()
            .setSize([4, 4]);
        nav.view._node._child[this._navigationDot].get()
            .setTransform(Transform.translate(this._navigationDot * 24 + 4, 0, 0));
        nav.view._node._child[this._navigationDot].get()
            .setOrigin([0, 0.5]);
        nav = this._navigationSurf[slide];
        nav.surf.addClass('active');
        nav.view._node._child[slide].get()
            .setSize([12, 12]);
        nav.view._node._child[slide].get()
            .setTransform(Transform.translate(slide * 24, 0, 0));
        nav.view._node._child[slide].get()
            .setOrigin([0, 0.5]);
    ...
    Slidershow.prototype.jumpToSlide = function jumpToSlide(slide, direction /* only for prev, next */ ) {
        var current = this.total - this.page >= this.total ? this.total - 1 : this.total - this.page <= 0 ? 0 : this.total - this.page,
        ...
        if (Math.abs(current - feature) !== (this.total - 1)) {
            this._sliders[next]._node.get()
                .setTransform(Transform.translate(this.options.width * -10, 0, 0), undefined);
            this._sliders[prev]._node.get()
                .setTransform(Transform.translate(this.options.width * -10, 0, 0), undefined);
        }
        this._sliders[feature]._node.get()
            .setTransform(Transform.translate(this.options.width * (current > feature ? 1 : -1), 0, 0));
        this._sliders[feature]._node.get()
            .setTransform(Transform.translate(0, 0, 0), this.options.transition);
        this._sliders[current]._node.get()
            .setTransform(Transform.translate(this.options.width * (current > feature ? -1 : 1), 0, 0), this.options.transition);
        if (Math.abs(current - feature) !== (this.total - 1)) {
            next = feature - 1 < 0 ? this.total - 1 : feature - 1;
            prev = feature + 1 >= this.total ? 0 : feature + 1;
            if (current > feature) {
                this._sliders[next]._node.get()
                    .setTransform(Transform.translate(this.options.width, 0, 0), undefined);
            }
            else {
                this._sliders[prev]._node.get()
                    .setTransform(Transform.translate(this.options.width * -1, 0, 0), undefined);
            }
        }
        this.page = slide + 1;
